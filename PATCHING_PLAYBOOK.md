...
- 2.1.280 request builders use `!eligible?void 0:config.display==="highlights"&&helper()?"omitted":config.display`. Default only the final display lookup to `"summarized"`; preserve eligibility checks and explicit display modes, including the highlights-to-omitted branch. The old matcher still reported ten streaming patches but missed this request default. The live native test reproduced an absent `thinking.display` before the fix and verified summarized requests and thinking rendered before the final answer afterward.
- 2.1.281 inserts a model-check helper between the thinking-enabled and display-eligibility assignments. Anchor the highlights matcher on the eligibility assignment and display expression, without requiring adjacency to the earlier environment check. CI passed on 2.1.280 and then failed on 2.1.281 at the same commit; compare downloaded versions and checksums before attributing a local/CI difference to the runner.
- 2.1.282 moves the provider check inside the model-eligibility helper, changing `enabled&&provider()&&supports(model)` to `enabled&&supports(model)`. Accept both forms and preserve their guards. The live native test reproduced a missing `thinking.display` while aggregate streaming coverage still reported ten patches.
